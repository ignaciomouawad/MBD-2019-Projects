...
for(user in 1:nrow(tran_critics)){
    ...
    abssum[user] = abs(wau)} 
sumabs <-sum(abssum)-1 #this will give us the sum absolute values of wau
...
sophia_avg <- mean(tran_critics$Sophia, na.rm = T) #averahe of sophias ratings
...
for (movie in 1:ncol(tran_critics)) {
  for(user in 1:nrow(tran_critics)){
    ...
    if(is.na(rum)==T| identical(tran_critics$Sophia, tran_critics[,user])){form <- 0} # we consider it 0 so that it will not be saved as the previous value in case other user did not rate movie, and 0 will not affect sum
    else{
      wau <- cor(tran_critics$Sophia,tran_critics[,user], use = "complete.obs")
  ...
  sophiaratings[movie] <- sophia_avg + (sumformvect[movie]/sumabs)  #ra+ (sum(r-ravg)*wau/sum(wau)) 
}
...
xy <- xy[,c(15,ncol(xy))]
xy[1:5,] #this will give us a table with top 5 movies
```
...
for(user in 1:ncol(transpose(critics))){
  if(identical(tran_critics$Sophia,tran_critics[,user])){ #to skip over sophia
    next
  }else{
...
xj <- xj[order(similar_users, decreasing = T, na.last = T),] #gives us top 5 order
xj[1:5,]
```