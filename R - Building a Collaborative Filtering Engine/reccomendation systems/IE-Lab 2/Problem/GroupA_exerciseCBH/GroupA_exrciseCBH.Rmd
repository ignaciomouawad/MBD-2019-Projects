...
author: Benjamin Chumaceiro | Ignacio Mouawad | Maria Joyce | Rosalia Contreras Moreira
  | Eduardo Cort Pons | Srishti Singh
date: "10/12/2019"
output:
...
rownames(unary_predict_table) <- rownames(question_topics)
...
rownames(unit_weight_predict_table) <- rownames(question_topics)
...
rownames(idf_predict_table) <- rownames(question_topics)
...
#### We took into considertion the fact that a new user would be interested in a question that has a decent answer. Therefore we decided to give answer votes an imprtance when considering what to show a new user like user 4. However, the spread between answer upvotes is big, and we came up with the idea to standardize the scores. This way, a question with an answer with many upvotes will not have an exaggerated impact on the recommendation.
...
#### Calculate the mean of the three idf values for each question, to be used in giving recommended questions for user 4. Since we have no idea abou the preferences of this user, we preferred this approach.
#### To come up with a final question score, apply a weighting to these results, split 90/10 between this mean and the standardized score. We reached this number after a careful thought process regarding the impact of a good answer. This way, the new user will be recommended questions as an average of all users with a push for good answers.
